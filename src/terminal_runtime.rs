...
use crate::terminal_scroll::{
    TerminalScrollSequenceTracker, TerminalScrollUpdate, TerminalVtScrollRegion,
};
...
impl SemanticPromptTracker {
    ...
    fn finish_payload(&mut self) {
        ...
        match command.first() {
            ...
            Some(b'B') if !self.ready_reported => {
...
impl NativeTerminalRuntime {
    ...
    pub fn interactive_shell_owns_foreground(&self) -> bool {
        self.pty
            .as_ref()
            .is_some_and(RuntimePty::interactive_shell_owns_foreground)
    }
    ...
    fn record_terminal_scroll_metadata(&mut self, bytes: &[u8]) {
        ...
        let full_viewport_region = scroll_update.region.is_none_or(|region| {
            region.top == 1
                && region.bottom == self.size.rows
                && region.left == 0
                && region.right == 0
        });
        let primary_full_viewport_scroll =
            full_viewport_region && self.terminal.active_screen().ok() == Some(Screen::Primary);
        if primary_full_viewport_scroll {
            // Primary-screen history growth is measured from libghostty's
            // scrollbar in `infer_primary_output_scroll`. Recording the same
            // line-feed scroll here would double-count it and let rapid prompt
            // output build an animation backlog.
            return;
        }
...
impl RuntimePty {
    fn spawn(size: TerminalGridSize, config: &TerminalSpawnConfig) -> Result<Self> {
        ...
        let interactive_shell = !config.direct_startup;
    ...
    #[cfg(unix)]
    fn interactive_shell_owns_foreground(&self) -> bool {
        if !self.interactive_shell {
            return false;
        }
        let Some(shell_process_group) = self.process_group_id else {
            return false;
        };
        let Some(descriptor) = self.master.as_raw_fd() else {
            return false;
        };
        // SAFETY: `descriptor` is the live master PTY owned by `self`.
        unsafe { libc::tcgetpgrp(descriptor) == shell_process_group }
    }
...
impl TerminalRendererModel {
    ...
    fn snapshot(&mut self, frame: &TerminalFrameSnapshot) -> NeovideRendererModelSnapshot {
        ...
        if let Some(rows) = inferred_scroll {
            // Primary-screen shell output can arrive faster than the spring can
            // settle (holding Return is the common case). Keep the newest
            // transition animated without accumulating a visual backlog behind
            // the PTY's current screen.
...
fn cap_primary_output_scroll_rows(rows: u64, _visible_rows: u64) -> isize {
    // A primary-shell frame may batch many prompt redraws between display-link
    // ticks. Animating that batch as a multi-row displacement makes the live
    // bottom line visibly race after the PTY has already reached its newest
    // prompt. The retained screen is current, so animate only the newest row.
    isize::from(rows != 0)
}
...
mod tests {
    ...
    #[test]
    fn tmux_prompt_tracker_deduplicates_ready_markers_per_prompt() {
        ...
        tracker.feed(b"\x1b]133;A\x1b\\prompt> \x1b]133;B\x07");
        assert_eq!(tracker.ready_generation, 2);
        tracker.feed(b"\x1b]133;B");
        tracker.feed(b";duplicate\x1b\\");
        assert_eq!(tracker.ready_generation, 2);

        tracker.feed(b"\x1b]133;D;0\x1b\\\x1b]133;B\x1b\\");
        assert_eq!(tracker.ready_generation, 3);
    }
    ...
    fn terminal_renderer_model_animates_primary_scrollback_growth() {
        ...
        assert_eq!(snapshot.windows[0].scroll_position, -1.0);
    }

    #[test]
    fn terminal_renderer_model_does_not_accumulate_primary_output_backlog() {
        let mut model = TerminalRendererModel::new(grid_size(4, 16));
        model.snapshot(&primary_frame_with_scrollbar(
            vec![row("one"), row("two"), row("three"), row("prompt>")],
            0,
        ...
        let first = model.snapshot(&primary_frame_with_scrollbar(
            vec![row("two"), row("three"), row("prompt>"), row("prompt>")],
            1,
            5,
        ));
        model.record_scroll_delta(24);
        let second = model.snapshot(&primary_frame_with_scrollbar(
            vec![row("three"), row("prompt>"), row("prompt>"), row("prompt>")],
            2,
            6,
        ));

        assert_eq!(first.windows[0].scroll_position, -1.0);
        assert_eq!(second.windows[0].scroll_position, -1.0);
    }
